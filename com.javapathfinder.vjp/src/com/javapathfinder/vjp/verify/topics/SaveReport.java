...
public class SaveReport implements SelectionListener{
  ...
  public void widgetSelected(SelectionEvent event) {
    ...
    //Are we sure that we want to overrite this file?
    if (file.exists()){
    ...
    try {
      ...
      for(Entry<String, Topic> entry : view.getResults().entrySet())
        out.println(entry.getValue().getContent());
      ...
      out.close();
        ...
        VJP.logError(e1.getMessage(),e1); //Unable to create the write stream
    } catch (IOException e1) {//Unable to create the new file
        MessageBox box = new MessageBox(shell, SWT.ICON_ERROR | SWT.OK);
